...
function upload( $remotepath, $localpath, $filename ){
    ...
    $ftp_server = $appconfig['sftp']['host'];
    $ftp_username = $appconfig['sftp']['username'];
    $ftp_userpass = $appconfig['sftp']['pw'];
...
function generateCSV($data,$type,$outputPath,$sftp) {
    //Generating timestamp CSV file name
    //$name = "klaviyo_upload_".$type."_".date("YmdHis").".csv";
    $name = sprintf( $appconfig['klaviyo']['filename'], $type, date("YmdHis") ); 
    $file = fopen( $name, "w" );
    $csv = array();
    array_push($csv, array_keys($data[0]));
    for ($i = 0; $i < sizeof($data); $i++) {
        array_push($csv, array_values($data[$i]));
    }
    foreach ($csv as $line) {
        fputcsv($file, $line);
    }
